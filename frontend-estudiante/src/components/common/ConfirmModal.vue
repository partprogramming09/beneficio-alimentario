  ...
  <div v-if="isOpen" class="modal-backdrop" @click.self="$emit('close')">
    <div :class="['modal-card', 'modal-' + type]">
      ...
      <div class="modal-footer">
        ...
        <button :class="['btn', confirmBtnClass]" :disabled="!canConfirm" @click="$emit('confirm')">{{ confirmText }}</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    confirmBtnClass() {
      const map = { danger: 'btn-danger', warning: 'btn-warning', success: 'btn-success' };
      return map[this.type] || 'btn-primary';
    },
  },
  watch: {
    isOpen(val) {
      if (val) document.addEventListener('keydown', this.onEscape);
      else document.removeEventListener('keydown', this.onEscape);
    },
  },
  beforeUnmount() {
...
.modal-icon-wrapper { flex-shrink: 0; }
